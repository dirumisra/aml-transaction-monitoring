...
-----------------------
Purpose : Train and evaluate Random Forest model on Gold layer data.
Layer   : ML Models — Phase 3
Input   : data/gold/aml_gold.parquet
Output  : artifacts/random_forest.pkl
Author  : Dhiru Misra
Version : 1.0.0
"""
# ==========================
...
# Constants
# ==========================
# Get the full path of current file and then its folder (where this script exists)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# Move 2 folders up from current folder to reach project root directory
PROJECT_ROOT = os.path.abspath(os.path.join(BASE_DIR, "..", ".."))

# Create full path for Gold layer dataset file
GOLD_FILE = os.path.join(PROJECT_ROOT, "data", "gold", "aml_gold.parquet")

# Create full path where trained model (.pkl) will be saved
ARTIFACT_PATH = os.path.join(PROJECT_ROOT, "artifacts", "random_forest.pkl")

# Target column (what we want to predict → fraud or not)
TARGET_COLUMN = "is_laundering"

# 20% data will be used for testing, 80% for training
TEST_SIZE = 0.2

# Fix randomness so results remain same every time (reproducibility)
RANDOM_STATE = 42

# Number of trees in Random Forest (starting value, can be tuned later)
N_ESTIMATORS = 100
...
# ==========================
def train_model(X_train, y_train):   # ✅ FIXED: use y_train (not y_test)
    """
    ...
    # Step 2: Print number of trees being used
    ...
    model = RandomForestClassifier(
        ...
        random_state=RANDOM_STATE,   # fix randomness → same results every run (reproducibility)
        class_weight="balanced",     # handle imbalance automatically
        n_jobs=-1                    # use all CPU cores (faster training)
    )

    # Step 4: Train model using training data
    model.fit(X_train, y_train)

    # Step 5: Print completion message
    logger.info("Model training complete ✅")

    # Step 6: Return trained model
    return model
